...
import fr.Boulldogo.WatchLogs.Discord.Commands.SetupSplitCommand;
...
public class SetupDiscordBot extends ListenerAdapter {
    ...
    public void startBot() throws LoginException {
        ...
        jdaBuilder.setActivity(Activity.playing(EmojiParser.parseToAliases(plugin.getConfig().contains("discord.activity_message") ? plugin.getConfig().getString("discord.activity_message") : ":eyes: See you with WatchLogs plugin")));
    ...
    private void registerCommands() {
        ...
        addCommand(new SetupSplitCommand(plugin));
    }
    ...
    public void sendDirectLogs(int id, String type, String data, String username, String worldName, String location, String nonTranslateAction, String serverName) {
        ...
        if(!serverName.equals("undefined")) {
            builder.addField(plugin.getLang().getString("discord.server-name"), serverName, true);
        }
        builder.setFooter(":white_check_mark: Generated by WatchLogs plugin by Boulldogo, type /project for project !");
        builder.setTimestamp(Instant.now());
        
        boolean splitEnableForLog = ((!ActionUtils.customActions.isEmpty() && ActionUtils.customActions.containsKey(nonTranslateAction)) ? (plugin.getConfig().getBoolean("discord.enable-split-log-system") && !plugin.getConfig().getString("splitted-discord-custom-logs." + getServerForAction(nonTranslateAction) + "." + nonTranslateAction).equals("-"))
        	 : (plugin.getConfig().getBoolean("discord.enable-split-log-system") && !plugin.getConfig().getString("splitted-discord-logs." + nonTranslateAction).equals("-")));
        String channelId = splitEnableForLog ? ((!ActionUtils.customActions.isEmpty() && ActionUtils.customActions.containsKey(nonTranslateAction)) ? getChannelIdForCustomAction(nonTranslateAction) : plugin.getConfig().getString("splitted-discord-logs." + nonTranslateAction)) : plugin.getConfig().getString("discord.live_logs_channel_id");
    ...
    private String getChannelIdForCustomAction(String actionName) {
    	if(ActionUtils.customActions.isEmpty() || !ActionUtils.customActions.containsKey(actionName)) return null;
    	
    	for(String server : plugin.getServerList()) {
    		if(plugin.getConfig().contains("splitted-discord-custom-logs." + server + "." + actionName)) {
    			return plugin.getConfig().getString("splitted-discord-custom-logs." + server + "." + actionName);
    		}
    	}
    	return null;
    }
    
    private String getServerForAction(String actionName) {
    	if(ActionUtils.customActions.isEmpty() || !ActionUtils.customActions.containsKey(actionName)) return null;
    ...
    private void displayBotInfo() {
        ...
        plugin.getLogger().info("[Discord-Module] Login with client: " + userName + "#" + discriminator + " (ID: " + userId + ")");
    }
}
